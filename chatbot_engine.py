...
class ChatbotEngine:
    ...
    def _generate_demo_response(self, user_input: str, persona: Dict[str, Any]) -> str:
        ...
        Incorporates memory callbacks to demonstrate context retention!
        """
        pid = persona["id"]
        turn_count = len(self.memory_history) // 2 + 1
        
        # Check if we have past context to callback to!
        memory_callback = ""
        if self.memory_history:
            prev_user_msgs = [m["content"] for m in self.memory_history if m["role"] == "human"]
            if prev_user_msgs:
                first_msg = prev_user_msgs[0]
                memory_callback = f" (And yes, I still remember you said '{first_msg}' in Question 1!)"
        ...
        if pid == "roastbot":
            roasts = [
                ...
                f"Judge, if I had a rupee for every time you asked something like that, I'd buy this stage and fire the panel!{memory_callback}",
                f"Is this your strategy to buzz me off stage? Asking questions that make the audience check their phones?{memory_callback}",
                f"Bold of you to say '{user_input}'. Did you spend all night preparing that or did it just slip out by accident?{memory_callback}",
            ]
            ...
            emoji_combos = [
                ...
                f"🔥⚡ ➡️ 🌶️🤪 ➡️ 🏆✨\n\n**Translation:** Hot take detected! That question was spicier than a street samosa!{memory_callback}",
                f"🧐📜 ➡️ 🤡🎪 ➡️ 🏃‍♂️💨\n\n**Translation:** The judges are examining the act, but it looks like a circus show!{memory_callback}",
            ]
            ...
            dialogues = [
                ...
                f"Kitne aadmi the, judge saab? Jo aise sawaal puchte hain? Yeh 'India's Got Latent' hai, Gabbar ka kila nahi!{memory_callback}",
            ]
